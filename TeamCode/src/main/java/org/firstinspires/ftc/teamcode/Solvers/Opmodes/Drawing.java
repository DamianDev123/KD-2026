...
public class Drawing {
    ...
    public static void drawGoal(Pose pose, String color) {
        if (packet == null) packet = new TelemetryPacket();
        packet.fieldOverlay().setStroke(color);
        Drawing.drawTarget(packet.fieldOverlay(), pose.copy());
    }
    ...
    public static void drawTarget(Canvas c, Pose t) {
        Pose goalPose = flip(FTCCoordinates.INSTANCE.convertFromPedro(Robot.getInstance().GoalPose));
        c.strokeLine(t.getX(),t.getY(), goalPose.getX(),  goalPose.getY());
    }
    static Pose flip(Pose pose){
