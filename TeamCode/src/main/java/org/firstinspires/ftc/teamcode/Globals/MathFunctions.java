...
public class MathFunctions {
    ...
    public static double getTimeOfFlight(double distance, double velocity, double angleFromVertical) {
        if (velocity <= 0 || distance <= 0) return Double.NaN;

        // Convert to angle from horizontal (physics frame)
        double angleHorizDeg = 90.0 - angleFromVertical;
        double angleRad = Math.toRadians(angleHorizDeg);

        double cosTheta = Math.cos(angleRad);

        // Prevent divide-by-zero or near-vertical shots
        if (Math.abs(cosTheta) < 1e-6) return Double.NaN;

        // Horizontal motion: x = v * cos(theta) * t
        return distance / (velocity * cosTheta);
    }
